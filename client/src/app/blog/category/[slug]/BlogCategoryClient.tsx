...
export default function BlogCategoryClient({ category }: Props) {
  const [posts, setPosts] = useState<BlogPost[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.get<{ items: BlogPost[] }>(`/blog/posts?categorySlug=${category.slug}&status=PUBLISHED`)
    ...
    <div className="min-h-screen bg-slate-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="bg-white p-8 rounded-3xl border border-slate-200/80 shadow-sm space-y-3">
          <span className="text-xs font-extrabold text-[#C9A227] uppercase tracking-widest block">Category Archive</span>
          <h1 className="text-3xl sm:text-4xl font-extrabold text-[#0F1A2E]">{category.name}</h1>
          {category.description && (
            <p className="text-sm sm:text-base text-slate-600 max-w-2xl">{category.description}</p>
          )}
        </div>
